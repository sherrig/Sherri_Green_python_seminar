#Sherri Green
#4/27/15
#Magnetic field through generator
...
#Approximate mag field in between rings to be uniform (assume it's a solenoid)
...
def approx_flux(phi):
    ...
    for i in xrange(int(S / dx)):
        for j in xrange(int(S / dy)):
            ...
            B = np.array((0,0,mu0*(2.0/.1)*I))                    #B field inside a solenoid is 4*pi*k/c in cgs. I think that translates to mu0*n*I in SI. the 2/.1 comes from n which is wraps per unit length -- the 2 rings are 10 cm apart and there are 2 of them?
            dF = np.dot(B, dA)
            Flux += dF
    return Flux
...
uniform_Flux = []
norm = approx_flux(0)
norm_gen = generatorFlux(0)
for i in range(int((2*pi)/dtheta)):
    try:
        phi.append(i*dtheta)
        generated_Flux.append(generatorFlux(i*dtheta) / norm_gen)
        uniform_Flux.append(approx_flux(i*dtheta) / norm)
    except KeyboardInterrupt:
        print 'Numerical:', generatorFlux(i*dtheta)
        print 'Theoretical:', approx_flux(i*dtheta)
plt.plot(phi, generated_Flux, 'b.')
plt.plot(phi, uniform_Flux, 'r*')
show()
